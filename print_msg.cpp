...
void print_join(int index, char *c) {   
    int connfd = client[index].fd;
    ...
    // show the topic
    memset(msg, '\0', MSG_SIZE);
    if (channels[c].topic.size() == 0) sprintf(msg, ":%s 331 %s %s :No topic is set\n", SERVER_NAME, nick_name, c);
...
void print_channel_users(int index, char *c) {
    int connfd = client[index].fd;
    char nick_name[NAME_MAX];
    strcpy(nick_name, client_info[index].nick_name);
